...
from runtime.forge.completion_audit import (
    DEFAULT_SLUG,
    OUTPUT_ROOT,
    build_chaser_forge_completion_audit,
    render_chaser_forge_completion_audit,
)
...
def _seed_evidence(vault: Path) -> None:
    ...
    for rel_path in DOC_RELATIVE_PATHS:
        _write(vault / rel_path, f"# {rel_path.stem}\n\nStatus: COMPLETE / VERIFIED\n")
    _write(
        vault / "06_AGENTS/Feature-Register.md",
        "### Chaser Forge\n\nStatus: COMPLETE / GOVERNED CHASER FORGE MARKETPLACE, PUBLISHED STATIC INDEX REGISTRATION, STATIC HOST UPLOAD RECEIPT, STATIC HOST UPLOAD HANDOFF, STATIC HOST PUBLICATION PROOF, HOSTED EXPORT BUNDLE, REMOTE DISTRIBUTION FOUNDATION, AND STUDIO UI VERIFIED\n",
    )
    _write(
        vault / "06_AGENTS/Feature-Fit-Register.md",
        "| Chaser Forge | COMPLETE / GOVERNED CHASER FORGE MARKETPLACE, PUBLISHED STATIC INDEX REGISTRATION, STATIC HOST UPLOAD RECEIPT, STATIC HOST UPLOAD HANDOFF, STATIC HOST PUBLICATION PROOF, HOSTED EXPORT BUNDLE, REMOTE DISTRIBUTION FOUNDATION, AND STUDIO UI VERIFIED |\n",
    )
    visual_report = {
        "ok": True,
        "summary": {
        ...
        "forge_source_group": {"status_counts": {}},
        "evidence": {
    ...
    _write(vault / VISUAL_QA_REPORT_RELATIVE_PATH, json.dumps(visual_report, indent=2) + "\n")
    ...
    _write(vault / MARKETPLACE_BRIDGE_VISUAL_QA_FALLBACK_REPORT_RELATIVE_PATH, json.dumps(marketplace_report, indent=2) + "\n")
    ...
    _write(vault / LIVE_STUDIO_CONTROL_PROOF_FALLBACK_REPORT_RELATIVE_PATH, json.dumps(live_report, indent=2) + "\n")
    operator_use_report = {
        "ok": True,
...
def test_completion_audit_marks_local_mvp_complete_when_proofs_and_registers_exist(tmp_path: Path) -> None:
    _seed_evidence(tmp_path)

    audit = build_chaser_forge_completion_audit(tmp_path, generated_at="2026-05-21T09:00:00Z")

    assert audit["ok"] is True
    assert audit["status"] == "COMPLETE / GOVERNED CHASER FORGE MARKETPLACE, PUBLISHED STATIC INDEX REGISTRATION, STATIC HOST UPLOAD RECEIPT, STATIC HOST UPLOAD HANDOFF, STATIC HOST PUBLICATION PROOF, HOSTED EXPORT BUNDLE, REMOTE DISTRIBUTION FOUNDATION, AND STUDIO UI REGISTERED"
    assert audit["panel_summary"]["local_mvp_implemented"] is True
    assert audit["proof_deck"]["ok"] is True
    assert audit["live_studio_control_proof"]["live_install_executed"] is True
    assert audit["operator_use_studio_proof"]["install_status_visible_after_refresh"] is True
    assert audit["operator_use_closeout_smoke"]["ok"] is True
    assert audit["local_marketplace_library_smoke"]["ok"] is True
    assert audit["local_marketplace_library_smoke"]["library_item_count"] == 1
    assert audit["remote_distribution_smoke"]["ok"] is True
    assert audit["remote_distribution_smoke"]["library_item_count"] == 1
    assert audit["hosted_marketplace_export_bundle_smoke"]["ok"] is True
    assert audit["hosted_marketplace_export_bundle_smoke"]["entry_count"] == 1
    assert audit["static_host_publication_smoke"]["ok"] is True
    assert audit["static_host_publication_smoke"]["file_count"] == 5
    assert audit["static_upload_handoff_smoke"]["ok"] is True
    assert audit["static_upload_handoff_smoke"]["file_count"] == 5
    assert audit["static_upload_receipt_smoke"]["ok"] is True
    assert audit["static_upload_receipt_smoke"]["file_count"] == 5
    assert audit["static_upload_receipt_smoke"]["upload_receipt_digest_sha256"] == "receipt"
    assert audit["published_static_index_registration_smoke"]["ok"] is True
    assert audit["published_static_index_registration_smoke"]["file_count"] == 5
    assert audit["published_static_index_registration_smoke"]["published_static_index_registration_digest_sha256"] == "registration"
    assert audit["registration"]["feature_register_marketplace_complete"] is True
    assert audit["registration"]["feature_fit_register_marketplace_complete"] is True
    assert audit["authority"]["canonical_mutation_allowed"] is False


def test_completion_audit_write_stays_under_workflow_proofs(tmp_path: Path) -> None:
    _seed_evidence(tmp_path)
    ...
    assert audit["ok"] is True
    assert audit["write_executed"] is True
    assert audit["markdown_path"] == f"{OUTPUT_ROOT.as_posix()}/{DEFAULT_SLUG}.md"
    assert audit["json_path"] == f"{OUTPUT_ROOT.as_posix()}/{DEFAULT_SLUG}.json"
    assert (tmp_path / audit["markdown_path"]).is_file()
    assert (tmp_path / audit["json_path"]).is_file()


def test_completion_audit_blocks_when_register_rows_are_missing(tmp_path: Path) -> None:
    _seed_evidence(tmp_path)
    (tmp_path / "06_AGENTS/Feature-Register.md").write_text("# no forge\n", encoding="utf-8")

    audit = build_chaser_forge_completion_audit(tmp_path, generated_at="2026-05-21T09:10:00Z")

    assert audit["ok"] is False
    assert "registration_check_failed:feature_register_mentions_chaser_forge" in audit["blockers"]
    assert "registration_check_failed:feature_register_marketplace_complete" in audit["blockers"]


def test_completion_audit_markdown_states_verified_marketplace(tmp_path: Path) -> None:
    _seed_evidence(tmp_path)
    audit = build_chaser_forge_completion_audit(tmp_path, generated_at="2026-05-21T09:15:00Z")

    output = render_chaser_forge_completion_audit(audit)

    assert "Chaser Forge Marketplace Completion Audit" in output
    assert "COMPLETE / GOVERNED CHASER FORGE MARKETPLACE, PUBLISHED STATIC INDEX REGISTRATION, STATIC HOST UPLOAD RECEIPT, STATIC HOST UPLOAD HANDOFF, STATIC HOST PUBLICATION PROOF, HOSTED EXPORT BUNDLE, REMOTE DISTRIBUTION FOUNDATION, AND STUDIO UI VERIFIED" in output
    assert "Marketplace status: COMPLETE / CHASEOS-OWNED LOCAL PUBLIC CATALOG, GOVERNED PUBLISHED STATIC INDEX REGISTRATION, GOVERNED STATIC HOST UPLOAD RECEIPT, GOVERNED STATIC HOST UPLOAD HANDOFF, GOVERNED STATIC HOST PUBLICATION PROOF, GOVERNED HOSTED EXPORT BUNDLE, GOVERNED REMOTE DISTRIBUTION FOUNDATION, AND GOVERNED INSTALL VERIFIED" in output
    assert "Remote Distribution smoke: true" in output
    assert "Static Host Publication smoke: true" in output
    assert "Static Upload Handoff smoke: true" in output
    assert "Static Upload Receipt smoke: true" in output
