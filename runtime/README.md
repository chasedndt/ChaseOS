...
### SiteOps / Website Workflow Index Layer
`runtime/siteops/`
- dry-run-only ChaseOS SiteOps production scaffold
...
- scoped artifacts under `07_LOGS/SiteOps-Runs/`, `07_LOGS/SiteOps-Audits/`, and `07_LOGS/SiteOps-Approvals/`
...
The 2026-05-03 RPGL target-profile plan pass added `chaseos runtime provider target-profile-plan [MODEL]`. It builds a portable candidate profile for the current or supplied model, preserves runtime fallback chains, keeps fallback enforcement observe-only by default, and can write only a proposal artifact under `07_LOGS/Agent-Activity/_rpgl_provider_target_profile_proposals/` plus a `needs_operator_approval` queue item. It does not write `runtime/providers/provider_target_profile.json`, edit model config, mutate provider/setup state, call providers, read secrets, drain queues, or restart gateways.
...
The 2026-04-28 config-validation pass added read-only `chaseos config validate` over `.chaseos/config.yaml`. It reports ready/blocked posture, allowed schema keys, invalid value/path issues, and secret-like key placement without mutating config or expanding Gate authority. This narrows final config/settings polish toward future Studio rendering rather than basic Runtime Shell validation.
...
- focused verification is green across:
  - `runtime/agent_bus/test_agent_bus.py`
  - `runtime/agent_bus/test_capabilities_router.py`
  - `runtime/agent_bus/test_bus_coordination_e2e.py`
...
| `siteops/*.py` + `siteops/catalog/**` + `siteops/tenants/**` + `siteops/registry/**` + `siteops/schemas/**` | `06_AGENTS/ChaseOS-SiteOps.md` + `06_AGENTS/SiteOps-Multi-User-Production-Architecture.md` + `runtime/COMMANDS.md` + `06_AGENTS/ChaseOS-CLI-Command-Reference.md` | When SiteOps production objects, tenant scope, dry-run behavior, command surface, audit/approval shape, or execution boundary changes |
...
| Logs and audits | `07_LOGS/` | Historical trace |
...
*Graph links auto-wired by vault_hygiene (2026-05-01): [[20260427-095156-strikezone-rss-coindesk]] . [[20260427-095156-strikezone-rss-decrypt]] . [[20260430__candidate-run-123]] . [[CODEX_CLI_LIVE_TEST_HANDOFF]] . [[DELIVERY-HEALTH]] . [[codex-live-subprocess-policy-block]] . [[codex-shell-policy-block]] . [[codex-stdout]] . [[grok_digest]] . [[grok_digest.template]] . [[perplexity_digest]] . [[perplexity_digest.template]] . [[research_export]] . [[youtube_summary]] . [[youtube_summary.template]]*


*Graph links auto-wired by vault_hygiene (2026-05-02): [[20260501-095138-strikezone-rss-coindesk]] . [[20260501-095138-strikezone-rss-decrypt]]*


*Graph links auto-wired by vault_hygiene (2026-05-04): [[2026-05-04-siteops-shadow-replay-candidate-run-123]] . [[shadow-replay-candidate-run-123]]*
