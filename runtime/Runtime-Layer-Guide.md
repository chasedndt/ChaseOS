...
- Output scope: draft artifacts in `07_LOGS/Developer-Briefs/`, audit in `07_LOGS/Agent-Activity/`, build log in `07_LOGS/Build-Logs/`, archive note in `99_ARCHIVE/Documentation-History/`
...
- Output scope: `runtime/acquisition/packs/` or `07_LOGS/Acquisition-Packs/`
...
`runtime/agent_bus/` holds the first durable dual-runtime coordination substrate for Hermes ↔ OpenClaw work.

Seeded artifacts:
- `runtime/agent_bus/Agent-Bus-Folder-Guide.md` — coordination substrate overview
- `runtime/agent_bus/task-packet.schema.json` — machine task packet schema
- `runtime/agent_bus/event.schema.json` — immutable event schema
- `runtime/agent_bus/heartbeat.schema.json` — runtime heartbeat schema
- `runtime/agent_bus/sqlite_schema.sql` — durable SQLite schema
- `runtime/agent_bus/examples/` — example packets
...
| `adapters/hermes.yaml` | `HERMES.md` + `.chaseos/hermes_config.yaml` | When Hermes shadow/runtime scope changes |
...
| `siteops/registry/**/*.json` | `06_AGENTS/ChaseOS-SiteOps.md` + `runtime/siteops/README.md` + `runtime/COMMANDS.md` | When SiteOps site profiles, provider profiles, workflow manifests, or Site Skill cards change |
