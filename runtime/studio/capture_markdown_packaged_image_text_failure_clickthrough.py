"""Packaged Capture to Markdown image text failure-state runner."""
...
from runtime.studio.packaged_app_visual_qa import (
    ...
    build_packaged_capture_markdown_image_text_failure_clickthrough,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the packaged Capture to Markdown explicit image text failure-state proof.",
    )
    ...
    parser.add_argument("--run-token", default=None, help="Unique token for the failure proof sentinel.")
    parser.add_argument("--settle-seconds", type=float, default=10.0, help="Initial packaged app settle time.")
    parser.add_argument("--window-timeout-seconds", type=float, default=35.0, help="Additional action timeout budget.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence_slug = args.evidence_slug or datetime.now(timezone.utc).strftime(
        "%Y-%m-%d-capture-markdown-packaged-image-text-failure-states"
    )
    ...
    report = build_packaged_capture_markdown_image_text_failure_clickthrough(
        vault,
