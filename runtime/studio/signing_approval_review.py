"""Operator-review approval artifact for a future Studio signing proof.

This pass may write exactly one scoped approval artifact for the signing packet
produced by ``signing_approval_preview``. It does not consume that approval,
reserve the exact-once marker, read signing certificates, sign artifacts, write
signed outputs, mutate startup/autostart, launch Studio, call providers or
connectors, enqueue Agent Bus tasks, mutate Gate, execute workflows, use Git,
...
MODEL_VERSION = "studio.signing_approval_review.v1"
SURFACE_ID = "studio_signing_approval_review"
APPROVAL_RECORD_TYPE = "studio_signing_approval_artifact"
READY_STATUS = "ready_to_write_studio_signing_approval_artifact"
WRITTEN_STATUS = "studio_signing_approval_artifact_written_no_execution"
EXISTING_STATUS = "studio_signing_approval_artifact_existing_matching_no_execution"
CONSUMED_STATUS = "studio_signing_approval_consumed_by_execution_proof"
DENIED_STATUS = "studio_signing_approval_denied_no_execution"
BLOCKED_STATUS = "blocked_studio_signing_approval_review"
...
def _resolve_vault_relative_path(vault: Path, path_value: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"signing approval review path escapes vault: {path_value}") from exc
    return resolved
...
def _approval_artifact_matches(payload: dict[str, Any], packet_id: str, request_digest: str) -> bool:
    return (
        ...
        and payload.get("signing_certificate_read") is False
...
def _write_approval_artifact(path: Path, payload: dict[str, Any]) -> str:
    ...
    if path.exists():
        ...
        raise ValueError(f"signing approval artifact already exists with different content: {path}")
...
def _approved_payload(
    *,
    ...
    portable_zip = source.get("portable_zip") or {}
    installer_manifest = source.get("installer_manifest") or {}
    installer_marker = source.get("exact_once_marker") or {}
    ...
    return {
        ...
        "reason": reason or "Operator approved one future Studio signing proof pass.",
        "approval_scope": "one_signing_proof_only",
        "approved_installer_approval_packet_id": summary.get("installer_approval_packet_id")
        or material.get("installer_approval_packet_id"),
        "approved_installer_execution_marker_path": installer_marker.get("path")
        or material.get("installer_execution_marker_path"),
        "approved_installer_execution_marker_sha256": installer_marker.get("sha256")
        or material.get("installer_execution_marker_sha256"),
        "approved_unsigned_portable_zip_path": portable_zip.get("path") or material.get("portable_zip_path"),
        "approved_unsigned_portable_zip_sha256": portable_zip.get("sha256") or material.get("portable_zip_sha256"),
        "approved_installer_manifest_path": installer_manifest.get("path") or material.get("installer_manifest_path"),
        "approved_installer_manifest_sha256": installer_manifest.get("sha256")
        or material.get("installer_manifest_sha256"),
        "approved_signing_profile": material.get("signing_profile") or "operator-provided-code-signing-certificate",
        "approved_output_root": material.get("output_root") or SIGNING_OUTPUT_ROOT.as_posix(),
        ...
        "next_recommended_pass": NEXT_SIGNING_CONSUMPTION_DRY_RUN_PASS,
    }


def build_studio_signing_approval_review(
    vault_root: str | Path,
    *,
    ...
    """Review and optionally write the scoped Studio signing approval artifact."""
    ...
    preview_report = build_studio_signing_approval_preview(
        vault,
    ...
    preview = preview_report.get("signing_approval_packet_preview") or {}
    summary = preview_report.get("summary") or {}
    ...
    signing_execution_complete = bool(preview_checks.get("signing_execution_proof_complete"))
    ...
    if approval_packet_id and approval_packet_id != packet_id:
        blockers.append("Requested approval packet id does not match the current signing packet.")
    if not preview_report.get("ok"):
        blockers.extend(str(item) for item in preview_report.get("blockers") or [])
    if not bool(summary.get("signing_approval_preview_ready")):
        blockers.append("Signing approval preview is not ready.")
    if not packet_id or not request_digest:
        blockers.append("Signing approval packet id or request digest is missing.")
    if not bool(preview_checks.get("installer_execution_proof_complete")):
        blockers.append("Installer-build execution proof is not complete.")
    if not bool(preview_checks.get("portable_zip_present")) or not bool(preview_checks.get("portable_zip_hash_present")):
        blockers.append("Portable ZIP proof is missing or unhashed.")
    if not bool(preview_checks.get("installer_manifest_present")) or not bool(preview_checks.get("installer_manifest_hash_present")):
        blockers.append("Installer manifest is missing or unhashed.")
    if not bool(preview_checks.get("future_signing_output_paths_clear")) and not signing_execution_complete:
        blockers.append("Future signing output paths are not clear.")
    if bool(marker.get("exists")) and not signing_execution_complete:
    ...
    if approval_path is not None and approval_path.exists():
        ...
        if not existing_matches:
            blockers.append("Existing signing approval artifact does not match the current packet.")
    ...
    elif signing_execution_complete:
        status = CONSUMED_STATUS
        write_status = "approval_consumed_by_existing_signing_execution_proof"
        next_pass = NEXT_STARTUP_AUTOSTART_APPROVAL_PASS
    elif existing_matches:
        status = EXISTING_STATUS
        write_status = "existing_matching_approval_present"
        next_pass = NEXT_SIGNING_CONSUMPTION_DRY_RUN_PASS
    ...
    authority = {
        **dict(BLOCKED_AUTHORITY),
        ...
        "future_single_signing_proof_approved": bool(approval_artifact_written),
    ...
    return {
        ...
        "summary": {
            ...
            "future_single_signing_proof_approved": approval_artifact_written,
        ...
        "checks": {
            ...
            "future_output_paths_clear": bool(preview_checks.get("future_signing_output_paths_clear")),
            "signing_execution_proof_complete": signing_execution_complete,
            "portable_zip_present": bool(preview_checks.get("portable_zip_present")),
            "installer_manifest_present": bool(preview_checks.get("installer_manifest_present")),
            "no_execution_in_this_pass": True,
            "certificate_not_read": True,
        },
        "authority": authority,
        "blockers": blockers,
        "unverified": [
            ...
            "No signing execution, signed artifact output, or signature verification was attempted.",
...
def write_signing_approval_review_evidence(
    vault_root: str | Path,
    report: dict[str, Any],
    *,
    ...
    slug = evidence_slug or f"{datetime.now(timezone.utc).strftime('%Y-%m-%d')}-studio-signing-approval-review"
    ...
    lines = [
        "# Studio Signing Approval Review Evidence",
        "",
