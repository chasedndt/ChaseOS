"""Operator-review approval artifact for a future Studio startup/autostart proof.

This pass may write exactly one scoped approval artifact for the startup/
autostart packet produced by ``startup_autostart_approval_preview``. It does not
consume approval, reserve the exact-once marker, resolve host startup paths,
write Windows startup folders, register autostart, write registry/Start Menu/
desktop shortcuts, promote a release, launch Studio, call providers or
connectors, enqueue Agent Bus tasks, mutate Gate, execute workflows, use Git, or
...
MODEL_VERSION = "studio.startup_autostart_approval_review.v1"
SURFACE_ID = "studio_startup_autostart_approval_review"
APPROVAL_RECORD_TYPE = "studio_startup_autostart_approval_artifact"
READY_STATUS = "ready_to_write_studio_startup_autostart_approval_artifact"
WRITTEN_STATUS = "studio_startup_autostart_approval_artifact_written_no_host_mutation"
EXISTING_STATUS = "studio_startup_autostart_approval_artifact_existing_matching_no_host_mutation"
CONSUMED_STATUS = "studio_startup_autostart_approval_artifact_consumed_by_execution_proof"
DENIED_STATUS = "studio_startup_autostart_approval_denied_no_host_mutation"
BLOCKED_STATUS = "blocked_studio_startup_autostart_approval_review"
...
def _resolve_vault_relative_path(vault: Path, path_value: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"startup/autostart approval review path escapes vault: {path_value}") from exc
    return resolved
...
def _approval_artifact_matches(payload: dict[str, Any], packet_id: str, request_digest: str) -> bool:
    return (
        ...
        and payload.get("host_path_resolution_attempted") is False
...
def _write_approval_artifact(path: Path, payload: dict[str, Any]) -> str:
    ...
    if path.exists():
        ...
        raise ValueError(f"startup/autostart approval artifact already exists with different content: {path}")
...
def _approved_payload(
    *,
    ...
    signing_marker = source.get("signing_exact_once_marker") or {}
    return {
        ...
        "reason": reason or "Operator approved one future Studio startup/autostart proof pass.",
        "approval_scope": "one_startup_autostart_proof_only",
        "approved_signing_approval_packet_id": material.get("signing_approval_packet_id"),
        "approved_signing_execution_marker_path": signing_marker.get("path")
        or material.get("signing_execution_marker_path"),
        "approved_signing_execution_marker_sha256": signing_marker.get("sha256")
        or material.get("signing_execution_marker_sha256"),
        ...
        "approved_startup_mode": material.get("startup_mode"),
        "approved_target_platform": material.get("target_platform") or "windows",
        "approved_host_targets": material.get("candidate_host_targets") or [item.get("id") for item in host_targets],
        "host_target_previews": host_targets,
        ...
        "next_recommended_pass": NEXT_STARTUP_CONSUMPTION_DRY_RUN_PASS,
    }


def build_studio_startup_autostart_approval_review(
    vault_root: str | Path,
    *,
    ...
    """Review and optionally write the scoped Studio startup/autostart approval artifact."""
    ...
    preview_report = build_studio_startup_autostart_approval_preview(
        vault,
    ...
    preview = preview_report.get("startup_autostart_approval_packet_preview") or {}
    ...
    host_targets = [dict(item) for item in preview_report.get("host_target_previews") or []]
    preview_checks = preview_report.get("checks") or {}
    startup_execution_complete = bool(preview_checks.get("startup_autostart_execution_proof_complete")) or (
        preview_report.get("status") == EXECUTION_COMPLETE_STATUS
    )
    packet_id = str(preview.get("approval_packet_id") or summary.get("startup_autostart_approval_packet_id") or "")
    ...
    if approval_packet_id and approval_packet_id != packet_id:
        blockers.append("Requested approval packet id does not match the current startup/autostart packet.")
    if not preview_report.get("ok"):
        blockers.extend(str(item) for item in preview_report.get("blockers") or [])
    if not bool(summary.get("startup_autostart_approval_preview_ready")):
        blockers.append("Startup/autostart approval preview is not ready.")
    if not packet_id or not request_digest:
        blockers.append("Startup/autostart approval packet id or request digest is missing.")
    if not bool(preview_checks.get("signing_execution_proof_complete")):
        blockers.append("Signing approved execution proof is not complete.")
    ...
    if not bool(preview_checks.get("future_startup_output_paths_clear")) and not startup_execution_complete:
        blockers.append("Future startup/autostart output paths are not clear.")
    if bool(marker.get("exists")) and not startup_execution_complete:
        blockers.append("Future startup/autostart exact-once marker path already exists.")
    if not bool(preview_checks.get("host_paths_not_resolved")):
        blockers.append("Startup/autostart preview resolved host paths unexpectedly.")
    ...
    if approval_path is not None and approval_path.exists():
        ...
        if not existing_matches:
            blockers.append("Existing startup/autostart approval artifact does not match the current packet.")
    ...
    if not blockers and normalized_decision == "denied":
        ...
        status = CONSUMED_STATUS if startup_execution_complete else EXISTING_STATUS
        write_status = "existing_matching_approval_present"
        next_pass = NEXT_RELEASE_PROMOTION_APPROVAL_PASS if startup_execution_complete else NEXT_STARTUP_CONSUMPTION_DRY_RUN_PASS
        ...
        next_pass = NEXT_STARTUP_CONSUMPTION_DRY_RUN_PASS
    else:
        status = READY_STATUS
        next_pass = NEXT_OPERATOR_REVIEW_PASS
    ...
    authority = {
        **dict(BLOCKED_AUTHORITY),
        ...
        "future_single_startup_autostart_proof_approved": bool(approval_artifact_written),
        "startup_autostart_approval_preview_only": False,
    ...
    return {
        ...
        "summary": {
            ...
            "future_single_startup_autostart_proof_approved": approval_artifact_written,
        ...
        "checks": {
            ...
            "exact_once_marker_absent": (not bool(marker.get("exists"))) or startup_execution_complete,
            "future_output_paths_clear": bool(preview_checks.get("future_startup_output_paths_clear"))
            or startup_execution_complete,
            "startup_autostart_execution_proof_complete": startup_execution_complete,
            "signing_execution_proof_complete": bool(preview_checks.get("signing_execution_proof_complete")),
            ...
            "host_paths_not_resolved": bool(preview_checks.get("host_paths_not_resolved")),
            "host_mutation_blocked": bool(preview_checks.get("host_mutation_blocked_in_this_pass")),
        ...
        "unverified": [
            ...
            "No host startup, autostart, registry, Start Menu, desktop shortcut, or release mutation was attempted.",
            "No Studio launch, provider/connector call, Agent Bus write, Gate mutation, workflow execution, Git operation, or canonical writeback was attempted.",
        ],
        "writes_performed": writes_performed,
        "next_recommended_pass": next_pass,
    }
...
def write_startup_autostart_approval_review_evidence(
    vault_root: str | Path,
    report: dict[str, Any],
    *,
    ...
    slug = evidence_slug or f"{datetime.now(timezone.utc).strftime('%Y-%m-%d')}-studio-startup-autostart-approval-review"
    ...
    lines = [
        "# Studio Startup/Autostart Approval Review Evidence",
        "",
