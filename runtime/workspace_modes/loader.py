...
def _parse_simple_yaml(text: str) -> dict[str, Any]:
    ...
    while index < len(lines):
        ...
        if stripped.startswith("- "):
            if (
                current_map_key is not None
                and isinstance(result.get(current_key), dict)
                and current_map_key in result[current_key]
            ):
                nested_value = result[current_key][current_map_key]
                if not isinstance(nested_value, list):
                    nested_value = []
                    result[current_key][current_map_key] = nested_value
                nested_value.append(_coerce_scalar(stripped[2:]))
                continue
