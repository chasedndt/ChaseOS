...
def _gateway_target_launcher_contents(config: dict[str, Any]) -> str | None:
    ...
    return "\r\n".join(
        [
            ...
            'set "HERMES_GATEWAY_MODE=wsl-foreground"',
            ...
            *log_lines,
            'echo [%DATE% %TIME%] checking ChaseOS Hermes coordination daemon>>"%HERMES_GATEWAY_LOG%"',
            'del "%HERMES_DAEMON_PROBE%" >nul 2>&1',
            "powershell.exe -NoProfile -Command \"$rows = Get-CimInstance Win32_Process | Where-Object { $cmd = [string]$_.CommandLine; $_.ProcessId -ne $PID -and $cmd -and (($cmd -like '*hermes-daemon-loop.cmd*') -or (($cmd -like '*runtime.cli.main runtime daemon*') -and ($cmd -like '*--runtime hermes*'))) }; $rows | Select-Object -First 1 -ExpandProperty ProcessId\" >\"%HERMES_DAEMON_PROBE%\" 2>nul",
            'set "HERMES_DAEMON_ALREADY_RUNNING="',
            'for /f "usebackq delims=" %%P in ("%HERMES_DAEMON_PROBE%") do (',
            '  if not "%%P"=="" set "HERMES_DAEMON_ALREADY_RUNNING=%%P"',
            ')',
            'del "%HERMES_DAEMON_PROBE%" >nul 2>&1',
            'if defined HERMES_DAEMON_ALREADY_RUNNING (',
            '  echo [%DATE% %TIME%] Hermes daemon already running pid %HERMES_DAEMON_ALREADY_RUNNING%; launch skipped>>"%HERMES_GATEWAY_LOG%"',
            ') else (',
            '  echo [%DATE% %TIME%] launching ChaseOS Hermes coordination daemon>>"%HERMES_GATEWAY_LOG%"',
            '  if exist "%HERMES_DAEMON_LOOP%" (',
            '    start /b "ChaseOS Hermes Daemon" cmd.exe /d /c "%HERMES_DAEMON_LOOP%"',
            '  ) else if exist "%PYTHON_EXE%" (',
            '    start /b "ChaseOS Hermes Daemon" cmd.exe /d /c ""%PYTHON_EXE%" -m runtime.cli.main runtime daemon --runtime hermes --daemon-interval 30 --vault-root "%VAULT_ROOT%" --synthesize >>"%HERMES_DAEMON_LOG%" 2>&1"',
            '  ) else (',
            '    echo [%DATE% %TIME%] Hermes daemon skipped: no daemon loop or Python executable found>>"%HERMES_GATEWAY_LOG%"',
            '  )',
            ')',
            ...
            '  echo [%DATE% %TIME%] wake attempt %%I: wsl.exe -d %HERMES_WSL_DISTRO%>>"%HERMES_GATEWAY_LOG%"',
            '  wsl.exe -d %HERMES_WSL_DISTRO% %HERMES_WSL_USER_ARG% -- bash -lc "cd %HERMES_WSL_WORKDIR% && true" >>"%HERMES_GATEWAY_LOG%" 2>&1',
            ...
            ":hermes_wsl_ready",
            'echo [%DATE% %TIME%] checking for an existing Hermes gateway process>>"%HERMES_GATEWAY_LOG%"',
            'wsl.exe -d %HERMES_WSL_DISTRO% %HERMES_WSL_USER_ARG% -- pgrep -af "hermes gateway run" >>"%HERMES_GATEWAY_LOG%" 2>&1',
            'if "!ERRORLEVEL!"=="0" (',
            '  echo [%DATE% %TIME%] Hermes gateway already running; foreground launch skipped>>"%HERMES_GATEWAY_LOG%"',
            '  exit /b 0',
            ')',
            'echo [%DATE% %TIME%] WSL distro %HERMES_WSL_DISTRO% ready; launching Hermes gateway in foreground>>"%HERMES_GATEWAY_LOG%"',
            f"for /L %%I in (1,1,{retry_attempts}) do (",
            '  echo [%DATE% %TIME%] attempt %%I: wsl.exe -d %HERMES_WSL_DISTRO% %HERMES_WSL_USER_ARG%>>"%HERMES_GATEWAY_LOG%"',
            '  wsl.exe -d %HERMES_WSL_DISTRO% %HERMES_WSL_USER_ARG% -- bash -lc "%HERMES_WSL_COMMAND%"',
            '  set "HERMES_EXIT=!ERRORLEVEL!"',
            '  echo [%DATE% %TIME%] attempt %%I exit !HERMES_EXIT!>>"%HERMES_GATEWAY_LOG%"',
            '  if "!HERMES_EXIT!"=="0" exit /b 0',
            f"  if %%I LSS {retry_attempts} {_batch_ping_delay(retry_delay)}",
            ")",
            "exit /b !HERMES_EXIT!",
            "",
...
def _gateway_startup_launcher_contents(config: dict[str, Any]) -> str | None:
    ...
    return "\r\n".join(
        [
            "@echo off",
            f"rem ChaseOS managed startup delegate for {ui_label}",
            f'call "{_normalize_windows_path_text(target_path)}"',
            "",
        ]
    )
...
def build_runtime_startup_surfaces_report(runtime_id: str, *, probe_processes: bool = True) -> dict[str, Any]:
    record = load_lifecycle_record(runtime_id)
...
def build_startup_surfaces_report(runtime_id: str | None = None, *, probe_processes: bool = True) -> dict[str, Any]:
...
def build_startup_surface_settings_report(runtime_id: str | None = None, *, probe_processes: bool = True) -> dict[str, Any]:
