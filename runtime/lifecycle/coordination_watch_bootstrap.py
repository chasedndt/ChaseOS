...
def _build_windows_launcher_contents(runtime_id: str, runtime_name: str) -> str:
    return (
        ...
        "cd /d \"%VAULT%\"\r\n"
        f"\"%PYTHON%\" \"%VAULT%\\chaseos.py\" runtime coordination-watch-supervisor --runtime {runtime_id} --action start\r\n"
    )
...
def _build_wsl_launcher_contents(runtime_id: str, runtime_name: str, distro: str) -> str:
    ...
    return (
        "@echo off\r\n"
        f"wsl.exe -d {distro} -- bash -lc \"cd {repo_root} && python3 chaseos.py runtime coordination-watch-supervisor --runtime {runtime_id} --action start\"\r\n"
    )
...
def build_coordination_watch_bootstrap_plan(runtime_id: str) -> dict[str, Any]:
    ...
    if str(config.get("wsl_distro") or "").strip() and config.get("supervisor_host") != "windows":
        launcher_contents = _build_wsl_launcher_contents(
            runtime_id,
            str(config.get("runtime_name")),
            str(config.get("wsl_distro")),
        )
    else:
        launcher_contents = _build_windows_launcher_contents(runtime_id, str(config.get("runtime_name")))
...
def _build_handoff_script_contents(plan: dict[str, Any]) -> str:
    ...
    launcher_name = Path(str(plan.get("launcher_path") or "coordination-watch-start.cmd")).name
    ...
    return "\r\n".join(
        [
            ...
            f"$launcherPath = Join-Path $scriptRoot {_powershell_quote(launcher_name)}",
            f"$taskName = {_powershell_quote(task_name)}",
            "$registerArgs = @('/Create', '/SC', 'ONLOGON', '/TN', $taskName, '/TR', $launcherPath, '/F')",
