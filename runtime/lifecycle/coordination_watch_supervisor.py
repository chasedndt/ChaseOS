...
def _windows_host_path_for_wsl(path: Path) -> Path:
    """Return a WSL-invokable Windows-host path for repo-local Windows tools."""
...
def _windows_to_wsl_vault_path(path: str | Path) -> str:
    ...
    `C:\\Users\\alice\\chaseos` -> `<WINDOWS_USER_HOME_WSL>/<path>` for ANY user.
...
def _coordination_watch_python_executable(supervisor_host: str | None) -> str:
    if str(supervisor_host or "").strip().lower() == "windows":
        ...
        for candidate in candidates:
            if candidate.exists():
                return str(_windows_host_path_for_wsl(candidate))
        return str(_windows_host_path_for_wsl(candidates[0]))
...
def get_port_ownership_status(runtime_id: str) -> dict[str, Any]:
...
def get_supervised_coordination_watch_status(runtime_id: str) -> dict[str, Any]:
    """Return supervision status for a runtime, with a TTL result cache.

    Subprocess calls (tasklist.exe, wsl.exe, powershell.exe) are rate-limited
    to at most once per _STATUS_CACHE_TTL seconds per runtime_id.  Cached
    results are returned immediately without spawning any child process.
    """
    now = time.monotonic()
...
def _get_supervised_coordination_watch_status_live(runtime_id: str) -> dict[str, Any]:
    """Uncached implementation — always calls subprocess if a PID is present."""
    ...
    if pid > 0 and _pid_running(
        pid,
        platform=str(plan.get("platform") or ""),
        wsl_distro=str(plan.get("wsl_distro") or ""),
    ):
        status["running"] = True
        return status

    status["stale_state"] = True
    return status
