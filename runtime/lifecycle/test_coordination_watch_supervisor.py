...
def test_windows_hosted_supervisor_plan_uses_repo_windows_venv_from_wsl():
    ...
    assert hermes_python.endswith(("/.venv/Scripts/python.exe", "/.venv-win314/Scripts/python.exe", "/.venv-win/Scripts/python.exe"))
    assert openclaw_plan["platform"] == "windows"
    assert openclaw_python.endswith(("/.venv/Scripts/python.exe", "/.venv-win314/Scripts/python.exe", "/.venv-win/Scripts/python.exe"))
...
def test_windows_hosted_supervisor_python_prefers_existing_versioned_venv(monkeypatch):
    ...
    monkeypatch.setattr(supervisor.Path, "exists", lambda self: str(self).replace("\\", "/").endswith(".venv-win314/Scripts/python.exe"))

    resolved = supervisor._coordination_watch_python_executable("windows").replace("\\", "/")

    assert resolved.endswith("/.venv-win314/Scripts/python.exe")
...
def test_supervised_coordination_watch_status_reports_running_from_state_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("runtime.lifecycle.coordination_watch_supervisor.build_supervised_coordination_watch_plan", fake_build)
    monkeypatch.setattr("runtime.lifecycle.coordination_watch_supervisor._pid_running", lambda pid, **kwargs: pid == 9876)
