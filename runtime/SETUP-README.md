...
Discord control-plane validation is also no-secret. `chaseos setup discord validate --json` reads `.chaseos/discord_instance_bindings.yaml`, reports binding readiness for Studio and runtime lanes, and returns only presence/status fields. It must not print raw Discord IDs, webhook URLs, bot tokens, public keys, or other secret values.
