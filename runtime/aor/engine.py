...
def _resolve_workflow_handler(workflow_id: str) -> Any:
    if workflow_id == "operator_today":
        ...
        return run_operator_today
    if workflow_id == "operator_close_day":
        from runtime.workflows.operator_close_day import run_operator_close_day
        return run_operator_close_day
    if workflow_id == "graph_hygiene":
        from runtime.workflows.graph_hygiene import run_graph_hygiene
        return run_graph_hygiene
    if workflow_id == "graduate_ideas":
        from runtime.workflows.graduate_ideas import run_graduate_ideas
        return run_graduate_ideas
    if workflow_id == "browser_research":
        from runtime.workflows.browser_research import run_browser_research
        return run_browser_research
    if workflow_id == "siteops_execute":
        from runtime.workflows.siteops_execute import run_siteops_execute
        return run_siteops_execute
    if workflow_id == "developer_repo_explain_shadow":
        from .developer_shadow import run_developer_repo_explain
        return run_developer_repo_explain
    if workflow_id == "sbp_strikezone_digest":
        from runtime.workflows.sbp_strikezone_digest import run_sbp_strikezone_digest
        return run_sbp_strikezone_digest
    if workflow_id == "source_pack_builder":
        from runtime.acquisition.source_pack_builder import run_source_pack_builder
        return run_source_pack_builder
    if workflow_id == "strikezone_acquisition":
        from runtime.workflows.strikezone_acquisition import run_strikezone_acquisition
        return run_strikezone_acquisition
    if workflow_id == "trace_idea":
        from runtime.workflows.trace_idea import run_trace_idea
        return run_trace_idea
    if workflow_id == "openclaw_post_review_task":
        from runtime.workflows.openclaw_post_review_task import run_openclaw_post_review_task
        return run_openclaw_post_review_task
    if workflow_id == "hermes_review_execute":
    ...
    if workflow_id == "hermes_watch":
        ...
        return run_hermes_watch
    if workflow_id == "openclaw_watch":
        from runtime.workflows.openclaw_watch import run_openclaw_watch
        return run_openclaw_watch
    if workflow_id == "drift_scan":
        from runtime.workflows.drift_scan import run_drift_scan
        return run_drift_scan
    if workflow_id == "meeting_ingest_linker":
        from runtime.workflows.meeting_ingest_linker import run_meeting_ingest_linker
        return run_meeting_ingest_linker
    if workflow_id == "hermes_operator_today_shadow":
        from .hermes_shadow import run_hermes_operator_today_shadow
        return run_hermes_operator_today_shadow
    if workflow_id == "openai_operator_research_shadow":
        from runtime.workflows.openai_shadow import run_openai_operator_research_shadow
        return run_openai_operator_research_shadow
    if workflow_id == "os_hygiene_graph":
        from runtime.workflows.os_hygiene_graph import run_os_hygiene_graph
        return run_os_hygiene_graph
    if workflow_id == "hermes_research_synthesis":
    ...
    if workflow_id == "hermes_skill_review":
        from runtime.workflows.hermes_skill_review import run_hermes_skill_review
        return run_hermes_skill_review
    if workflow_id == "archon_watch":
        from runtime.workflows.archon_watch import run_archon_watch
        return run_archon_watch
    if workflow_id == "agent_runtime_governance_audit":
        from runtime.workflows.agent_runtime_governance_audit import run_agent_runtime_governance_audit
        return run_agent_runtime_governance_audit
    if workflow_id == "ventureops_ai_runtime_security_audit":
        from runtime.workflows.ventureops_ai_runtime_security_audit import run_ventureops_ai_runtime_security_audit
        return run_ventureops_ai_runtime_security_audit
    if workflow_id == "mission_chase_ai_runtime_governance_kit":
        from runtime.workflows.missions.mission_chase_ai_runtime_governance_kit import (
            run_mission_chase_ai_runtime_governance_kit,
        )
        return run_mission_chase_ai_runtime_governance_kit
    return None
...
def _stage_run(
    ...
    operator_today, operator_close_day, graph_hygiene, graduate_ideas,
    strikezone_acquisition, review-bus workflows, and other explicitly wired
    runtime handlers. Unregistered workflows still fail closed at dispatch.
    """
