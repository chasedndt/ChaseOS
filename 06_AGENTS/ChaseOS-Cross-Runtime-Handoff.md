...
- Hermes startup: Windows Startup now delegates to `<HERMES_HOME>/<path>`; the live target launcher was updated to enter WSL `Ubuntu` as user `chaseos`, run from `<VAULT_ROOT>`, retry, and log to `<HERMES_HOME>/<path>`. Post-logon proof remains unverified.
...
- Hermes WSL startup still needs post-logon evidence from `<HERMES_HOME>/<path>`; current WSL probe from shell recorded `Wsl/Service/E_ACCESSDENIED`.
