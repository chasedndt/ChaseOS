...
- `00_HOME/Assistant-Contract.md` — agent behavior contract
...
A system that knows the user's domain, priorities, and preferences can tailor its suggestions, context loading, and reasoning without being re-briefed each session. The user does not need to explain what StrikeZone is, what their trading style is, or what writeback means in every session.

**Canonical home:**
- `~/.claude/memory/user_chase_profile.md` — current user profile (Claude lane)
- `00_HOME/Now.md` — sprint-level priorities (read every session)
- `00_HOME/Principles.md` — personal operating doctrine
- `00_HOME/Operating-System.md` — full 18-domain OS definition
...
- Conversation records live under a declared log destination such as `07_LOGS/Conversations/` and are referenced by source hashes, target paths, retention class, and privacy scope.
...
- Every future write of conversation history must also create or reference Layer E audit evidence in `07_LOGS/Agent-Activity/`.
...
**Canonical home:**
- `07_LOGS/Build-Logs/` — engineering and documentation session logs
- `07_LOGS/Agent-Activity/` — agent-initiated action logs
- `07_LOGS/Daily/` — daily notes
- `07_LOGS/Trading-Weekly/` — weekly reviews
...
Tier 1 — Runtime-local incident
  ...
  Logged in: 07_LOGS/Agent-Activity/ or runtime execution log.
...
| B | User-Specific Operating Memory | Persistent, evolving | User-specific | `~/.claude/memory/`, `00_HOME/` |
...
| E | Execution-History/Audit Memory | Permanent, append-only | System-wide history | `07_LOGS/`, `99_ARCHIVE/` |
...
- Build session outputs → go in `07_LOGS/Build-Logs/`
